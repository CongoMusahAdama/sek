    ...
    >
      <div className="toast-content">
        ...
        <div className="toast-text">
          <p>
            <strong>{toast}</strong> added to order
          </p>
          <button className="toast-view-link">View Order</button>
        </div>
        <button
...
const NavCartButton = () => {
  ...
  return (
    ...
    >
      <span className="nav-icon-label">Order</span>
      ...
      <header className={`header ${isScrolled || !isHome ? "scrolled" : ""}`}>
        <div className="container flex items-center justify-between">
          ...
          <div className="nav-icons">
            ...
            {/* Admin/Auth link removed for public release */}
    ...
    <div className="collections-page">
      ...
      <section className="section-padding container">
        ...
        <div className="collections-main-grid">
          ...
          >
            <img src="/hero1.png" alt="Sekina Signature Selection" />
            <div
              className="hero-overlay"
              style={{
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[var(--brand-red)] text-white py-3 rounded-md font-bold mt-4 hover:bg-[#002010] transition-colors"
          >
...
const AdminProducts = ({
  ...
  const handleSubmit = async (e) => {
    ...
    const finalProdData = {
      ...
      sku: `MEAL-${Date.now()}` // Auto-generate SKU to satisfy backend uniqueness if needed
    };
              ...
              <div style={{ display: "flex", gap: "0.25rem" }}>
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    style={{
                      background: currentPage === i + 1 ? "var(--brand-red)" : "white",
...
const ReceiptModal = ({ order, onClose }) => {
  ...
  const handleDownloadAndShare = async () => {
    ...
    try {
      ...
      const waText = `Hello ${order.customer}, thank you for your order from Sekina Special! We've attached your digital receipt RC-${order.orderId?.split("-")[1]?.toUpperCase() || order._id} for your records. Enjoy your meal! 😋🍛`;
        ...
        >
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            >
              <h2
                className="serif"
                style={{ color: "var(--brand-red)", fontSize: "1.8rem", margin: "0" }}
              >
                Sekina Special
              </h2>
              <p
            ...
            >
              Enjoy your meal and thank you for your patronage!
            </div>
          </div>
        </div>
...
const AdminOrders = ({ orders, addOrder, updateOrder, products }) => {
  ...
  const filteredOrders = orders.filter(o => {
    const searchLow = adminSearch.toLowerCase();
    const matchesCustomer = o.customer?.toLowerCase().includes(searchLow);
    const matchesId = o.orderId?.toLowerCase().includes(searchLow) || o._id?.toLowerCase().includes(searchLow);
    
    // Search by item name or SKU
    const matchesItems = o.items?.some(item => {
      const nameMatch = item.name?.toLowerCase().includes(searchLow);
      const prod = products.find(p => p.name === item.name);
      const skuMatch = prod?.sku?.toLowerCase().includes(searchLow);
      return nameMatch || skuMatch;
    });
  ...
  return (
    <div className="dashboard-view fade-in">
      <header className="admin-header">
        <div className="page-title">
          <h1 className="serif">Sales Log</h1>
          <p>Manual orders for WhatsApp & Walk-ins — {filteredOrders.length} Records</p>
        </div>
        <div className="admin-search-wrap" style={{ position: 'relative', flex: 1, maxWidth: '300px' }}>
          <Search size={16} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
          <input 
            type="text" 
            placeholder="Search by ID, Customer or SKU..." 
            value={adminSearch}
            onChange={(e) => {
              setAdminSearch(e.target.value);
              setCurrentPage(1);
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            <div className="modal-header-premium">
              <div>
                <h2 className="serif">Record New Order</h2>
                <p>Add a manual entry for walk-in or WhatsApp orders.</p>
              </div>
              <button
                onClick={() => setShowModal(false)}
                className="sidebar-minimize-toggle"
            ...
            <form onSubmit={handleSubmit} className="admin-modal-form">
              <div className="modal-content-scroll">
                ...
                <div className="form-group">
                  ...
                  <input
                    type="text"
                    placeholder="e.g. Accra, Osu or Kitchen Pickup"
                ...
                <div className="form-group">
                  ...
                  >
                    {products.length > 0 ? (
                      ...
                      <option disabled>No meals available</option>
                    )}
                  </select>
                </div>
                <div
            ...
            <tbody>
              {currentItems.map((order, index) => (
                <tr key={order._id || order.id}>
                  ...
                  <td
                    data-label="Order ID"
                    style={{ fontWeight: 700, color: "var(--brand-red)" }}
                  >
            ...
            <tbody>
              {currentItems.map((order, index) => (
                <tr key={order._id || order.id}>
                  ...
                  <td data-label="Customer">{order.customer}</td>
                  <td data-label="Meal">{order.items[0]?.name || "N/A"}</td>
              ...
              <div className="payment-instruction-box">
                ...
                <div className="social-payment-support" style={{ marginTop: '1.5rem', borderTop: '1px solid #e2e8f0', paddingTop: '1rem' }}>
                  ...
                  <div className="flex gap-4" style={{ marginTop: '0.5rem' }}>
                    <a href="https://www.tiktok.com/@sekinaspecial" target="_blank" rel="noreferrer" title="TikTok"><span style={{ fontWeight: 800 }}>T</span></a>
                    <a href="https://www.instagram.com/sekinaspecial" target="_blank" rel="noreferrer" title="Instagram"><Instagram size={20} /></a>
                    <a href="https://www.facebook.com/sekinaspecial" target="_blank" rel="noreferrer" title="Facebook"><Facebook size={20} /></a>
                    <a href="https://www.snapchat.com/add/sekinaspecial" target="_blank" rel="noreferrer" title="Snapchat"><Ghost size={20} /></a>
                  </div>
                </div>
              </div>
...
const App = () => {
  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      // Force specific categories part of the brand - ignore API categories for home display
      setCategories([
        "Waakye",
        "Fried rice",
        "Plain rice and Nkotomire stew",
      ...
      const defaultMeals = [
        { _id: 'w1', name: 'Waakye', price: 35, category: 'Waakye', image: '/waakye.png', badge: 'Popular', description: 'Rice and beans served with shito, wele, egg, and plantain.' },
        { _id: 'f1', name: 'Fried rice', price: 40, category: 'Fried rice', image: '/fried rice.png', badge: 'Hot', description: 'Ghanaian style fried rice with spicy chicken.' },
        { _id: 'n1', name: 'Plain rice and Nkotomire stew', price: 45, category: 'Plain rice and Nkotomire stew', image: '/plain rice and kontomire stew.png', badge: 'Healthy', description: 'Plain rice with delicious spinach and melon seed stew.' },
        { _id: 'a1', name: 'Angwa mu', price: 30, category: 'Angwa mu', image: '/meal_angwa.png', badge: 'Local', description: 'Authentic oiled rice with sardines and hot pepper.' },
        { _id: 'i1', name: 'Indomie', price: 25, category: 'Indomie', image: '/indomie.png', badge: 'Fast', description: 'Stir-fried noodles with veggies and fried egg.' },
        { _id: 'j1', name: 'Jollof rice', price: 40, category: 'Jollof rice', image: '/meal_jollof.png', badge: 'Classic', description: 'World-famous smoky Ghanaian jollof rice with grilled chicken.' },
        { _id: 'r1', name: 'Red Red', price: 30, category: 'Red Red', image: '/meal_redred.png', badge: 'Local', description: 'Fried plantain with black-eyed beans stew and gari.' }
      ];
      ...
      if (prodRes.success && prodRes.data?.length > 0) {
        // Enforce images for API products by matching with our verified defaults
        const enrichedApi = prodRes.data.map(p => {
          ...
          if (isInvalidImg) {
            ...
            // 2. Try fuzzy match (if name contains a primary meal keyword)
            if (!match) {
      ...
      setProducts(uniqueList);
        ...
        // Deduplicate and merge categories from products - REMOVED since we want forced categories
        // setCategories(...) 
        ...
        <ModalProvider>
          <div className="app">
            ...
            <Routes>
              ...
              <Route
                path="/checkout"
                element={<CheckoutPage addOrder={addOrder} products={products} />}
              />
              <Route
                path="/admin/*"
                element={
